...
module.exports = {
    general: {
        ...
        run: async(client, kmsg, args, manager)=>{
            ...
            await mta_info.getServerByIp(args[0])
            .then(async(servers) => {
    ...
    slachcmd: {
        ...
        run: async(client, interaction)=> {
            let data = interaction.options.getString("ip")
            let mta_info = new client.pkgs.mta()
            await mta_info.getServerByIp(data)
            .then(async(servers) => {
                let data = Generator(servers, `MTA SA SERVER LIST`, `**|| Power By KMCodes Top1 Codeing Server ||**`)
                const button1 = new MessageButton()
                .setCustomId('previousbtn')
